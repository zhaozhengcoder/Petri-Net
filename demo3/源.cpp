...
public:
	...
	void calu_avg_delay_phase12(int beg, int end) {
		float in = 0;
		float out = 0;
		float sum = 0;
		for (int i = beg; i <= end; i++) {
			//in = in + In_phase1_p13[i] + In_phase1_p33[i];
			//out = out + Out_phase1_p13[i] + Out_phase1_p33[i];
			if (Out_phase1_p13[i] == 0) {
				sum = sum + In_phase1_p13[i];
			}
		}
		cout << "phase 1 平均等待时间 ：" << ((in - out) * avg_wait_time) / (in) << endl;
	}
	...
	void calu_avg_delay_phase2(int beg, int end) {
		...
		float out = 0;
		for (int i = beg; i <= end; i++) {
			in = in + In_phase2_p12[i] + In_phase2_p32[i];
			out = out + Out_phase2_p12[i] + Out_phase2_p32[i];
		}
		cout << "phase 2 平均等待时间 ：" << ((in - out) * avg_wait_time) / (in) << endl;
	}
	...
	void calu_avg_delay_phase3(int beg, int end) {
		...
		float out = 0;
		for (int i = beg; i <= end; i++) {
			in = in + In_phase3_p23[i] + In_phase3_p43[i];
			out = out + Out_phase3_p23[i] + Out_phase3_p43[i];
		}
		cout << "phase 3 平均等待时间 ：" << ((in - out) * avg_wait_time) / (in) << endl;
	}
	...
	void calu_avg_delay_phase4(int beg, int end) {
		...
		float out = 0;
		for (int i = beg; i <= end; i++) {
			in = in + In_phase4_p22[i] + In_phase4_p42[i];
			out = out + Out_phase4_p22[i] + Out_phase4_p42[i];
		}
		cout << "phase 4 平均等待时间 ：" << ((in - out) * avg_wait_time) / (in) << endl;
	}
...
void act(Graph & g, Config & config,Time &t,int run_times, int times = 3) {
	...
	//float out1=0;
	//for (int i = beg; i <= end; i++) {
	//	in1 = in1 + t.In_phase1_p13[i] + t.In_phase1_p33[i];
	//	out1 = out1 + t.Out_phase1_p13[i] + t.Out_phase1_p33[i];
	//}
	//cout << "phase 1 平均等待时间 ：" << ((in1 - out1) * 45)/(in1) << endl;
